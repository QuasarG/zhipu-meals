...
| `watchGapMs` | number | 守望间隔毫秒，默认 5000（低于限流器回血速率，可永不停止） |
| `sweepGapMs` | number | 扫荡期拉菜单步进毫秒，默认 500（约 10 发短脉冲） |
| `requestGapMs` | number | 兜底请求步进毫秒，默认 500 |
...
所有参数经实测标定（ratehold.js / probe-concurrent.js 可复测）：

| 实测项 | 结论 |
|---|---|
| 串行 GET 持续 0.5 发/秒 | 第 31 发触发限流 |
| 串行 GET 持续 1 发/秒 | 第 25 发触发限流 |
| 串行 GET 5 秒/发（0.2 发/秒） | 持续 2 分钟全过——**低于回血速率，永不触发** |
| 并发齐发 5~40 发 | **全部通过**——限流器只看相邻间隔，不拦并发突刺 |
| 下单 POST 连发 16 发 | 全部接受（仅业务码拒绝重复下单） |
| 限流冷却时长 | 约 13 分钟，期间重试会续期 |

推测限流器为令牌桶：容量 ≈24-30，回血 ≈0.25-0.5 发/秒。

据此设计三段变速：

- **守望**（周五 `activeStartHour` 起）：**5 秒/发单点探测，永不停歇**——
  0.2 发/秒低于回血速率，理论上无限打也不触发；放餐发现延迟 ≤5 秒
- **拉菜单**（探测到放餐）：步进降到 500ms 连拉 5 天 × 2 餐共 10 发
  短脉冲（桶内余额足够），约 5 秒拉完
- **下单**：**Promise.all 并发齐发**（POST 实测不限流），10 单同一瞬间
  打出；失败目标换候选后再齐发一批（最多两轮）
...
频控阈值全部经实测标定（见上节「实测限流画像」）：

- **周一 ~ 周四**：完全静默长眠，**零请求**，一觉睡到周五
- **周五 `activeStartHour`（默认 13:00）**：准点醒来进入**守望模式**——
  **5 秒/发单点探测，永不停歇**（低于限流器回血速率，理论上永不触发），
  游标轮转覆盖全部日期×餐次，放餐发现延迟 ≤5 秒
- **探测到放餐 → 扫荡**：
  1. 拉菜单步进降到 500ms，连拉 5 天 × 2 餐共 10 发短脉冲（约 5 秒）
  2. 逐天查订单防重复（已点的/手动点的跳过）
  3. 按稀缺度排序后 **Promise.all 并发齐发下单**；失败目标（库存被截）
     进死名单换候选再齐发一批（最多两轮）
- **周六 ~ 周日**：全部完成后零请求复查（30 分钟节奏）；手动取消某顿
  会触发自动补抢
- 周一 0 点起重新入眠，循环往复

若实测放餐时间是周五固定某点（如 16:00），可把 `activeStartHour` 调整为
`15` 收窄守望时长。
...
守望模式（5s/发单点探测，永不停止，游标轮转日期×餐次）
 ├─ 未放餐 → 5 秒后再探测（延迟 ≤5 秒）
 └─ 探测到放餐 → 立即扫荡
扫荡模式
 ├─ 拉菜单（500ms/发 ×10）→ 查订单防重复（一天 1 发）
 │   ├─ 已有订单（含手动点的）→ 跳过
 │   └─ 未点 → 按稀缺度排序
 ├─ 并发齐发下单（POST 不限流）；失败换候选再齐发（≤2 轮）
 ├─ 命中限流 → 整轮静默退避（默认 15 分钟，防续期）
