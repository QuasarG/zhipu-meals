...
- **探测到菜单放出**：立即切换**扫荡模式**，两阶段极速下单：
  - **阶段一（⚡极速）**：逐日期拉菜单途中，撞见稀缺候选
    （余量 < `scarceThreshold`，默认 100 即两位数）**跳过订单预查直接下单**——
    稀缺款放出时就是两位数库存，竞争窗口以秒计，最快 ~8 秒锁定
  - **阶段二（稳妥）**：剩余目标按稀缺度升序排队，先查订单防重复，
    逐顿下单；失败（库存被截/已点）自动换候选重试一轮
