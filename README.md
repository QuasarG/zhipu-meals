...
| `pollIntervalMs` | number | 活跃窗口内轮询间隔毫秒，默认 60000（1 分钟） |
...
- **周五 `activeStartHour`（默认 13:00）**：准点醒来，每 1 分钟一轮高频探测，
  放餐瞬间（延迟 ≤ 1 分钟）即完成全部下单
- **周六 ~ 周日**：窗口保持开启——全部点完后每轮零请求，若手动取消了某顿，
  30 分钟内自动补抢
- 周一 0 点起重新入眠，循环往复

一周总请求量：活跃窗口内几十发 + 周末零星复查，连频控的边都摸不到。
...
每轮巡检（活跃窗口内 1 分钟/轮）
 ...
 └─ 本周全部完成 → 周末每 30 分钟零请求复查（支持取消后补抢）→ 周一重新入眠
```
